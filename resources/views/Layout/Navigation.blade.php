<nav>

    <div class="navigation">
        
            <div class="menu d-flex justify-content-between align-items-center">
                <div class="nav-logo">
                    <a href="/">
                        <img src="/assets/img/LOGO.png" alt="Infinitech Advertising Logo">
                        {{-- <span class="logo-title">Infinitech<span class="logo-sub-title">advertising Corporation</span></span> --}}
                    </a>
                </div>
                <span>
                    <a class="icon-menu" data-bs-toggle="offcanvas" href="#offcanvasExample" role="button" aria-controls="offcanvasExample">
                        <i class="fa-solid fa-bars-staggered"></i>
                    </a>
                </span>
                <div class="nav-menu text-left">
                    <ul>
                        <li class="menu-item" data-link="/about-us"><a href="/about-us">About Us</a></li>
                        <li class="menu-item" data-link="/services"><a href="/services">Services</a></li>
                        <li class="menu-item" data-link="/solution"><a href="/solution">Solutions</a></li>
                        <li class="menu-item" data-link="/testimonials"><a href="/testimonials">Testimonials</a></li>
                        <li class="menu-item" data-link="/news-blogs"><a href="/news-blogs">Blogs</a></li>
                        {{-- <li class="menu-item" data-link="/our-pricing"><a href="/our-pricing">Pricing</a></li> --}}
                        <li class="menu-item" data-link="/contact-us"><a href="/contact-us">Contact Us</a></li>
                        <li class="menu-item">
                            <a href="/get-proposal">
                                <button class="button">
                                    <span>GET A QUOTE</span>
                                    <svg width="34" height="34" viewBox="0 0 74 74" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="37" cy="37" r="35.5" stroke="white" stroke-width="3"></circle>
                                        <path d="M25 35.5C24.1716 35.5 23.5 36.1716 23.5 37C23.5 37.8284 24.1716 38.5 25 38.5V35.5ZM49.0607 38.0607C49.6464 37.4749 49.6464 36.5251 49.0607 35.9393L39.5147 26.3934C38.9289 25.8076 37.9792 25.8076 37.3934 26.3934C36.8076 26.9792 36.8076 27.9289 37.3934 28.5147L45.8787 37L37.3934 45.4853C36.8076 46.0711 36.8076 47.0208 37.3934 47.6066C37.9792 48.1924 38.9289 48.1924 39.5147 47.6066L49.0607 38.0607ZM25 38.5L48 38.5V35.5L25 35.5V38.5Z" fill="white"></path>
                                    </svg>
                                </button>
                            </a>
                        </li>
                    </ul>
                </div>
                
                
                
            <div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
                <div class="offcanvas-header">
                    <div class="nav-logo d-flex align-items-center">
                        <img src="../assets/img/logooo.png" alt="">
                    </div>
                    <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                </div>
                <div class="offcanvas-body">
                    <ul class="list-unstyled mb-xl-5">
                        <li class="list-item" data-link="/about-us"><a href="/about-us">About Us</a></li>
                        <li class="list-item" data-link="/services"><a href="/services">Services</a></li>
                        <li class="list-item" data-link="/solution"><a href="/solution">Solutions</a></li>
                        <li class="list-item" data-link="/testimonials"><a href="/testimonials">Testimonials</a></li>
                        <li class="list-item" data-link="/news-blogs"><a href="/news-blogs">Blogs</a></li>
                        <li class="list-item" data-link="/  contact-us"><a href="/contact-us">Contact Us</a></li>
                        <li class="list-item">
                            <a href="/get-proposal">
                                <button class="button">
                                    <span>GET A QUOTE</span>
                                    <svg width="34" height="34" viewBox="0 0 74 74" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="37" cy="37" r="35.5" stroke="white" stroke-width="3"></circle>
                                        <path d="M25 35.5C24.1716 35.5 23.5 36.1716 23.5 37C23.5 37.8284 24.1716 38.5 25 38.5V35.5ZM49.0607 38.0607C49.6464 37.4749 49.6464 36.5251 49.0607 35.9393L39.5147 26.3934C38.9289 25.8076 37.9792 25.8076 37.3934 26.3934C36.8076 26.9792 36.8076 27.9289 37.3934 28.5147L45.8787 37L37.3934 45.4853C36.8076 46.0711 36.8076 47.0208 37.3934 47.6066C37.9792 48.1924 38.9289 48.1924 39.5147 47.6066L49.0607 38.0607ZM25 38.5L48 38.5V35.5L25 35.5V38.5Z" fill="white"></path>
                                    </svg>
                                </button>
                            </a>
                        </li>
                    </ul>


                    <ul class="list-contact d-block text-start">
                        <li><a href=""><i class="fa-solid fa-phone"></i>(02) 123 - 456 - 7890</a></li>
                        <li><a href=""><i class="fa-solid fa-mobile"></i> (63+) 9924401097</a></li>
                        <li><a href=""><i class="fa-brands fa-viber"></i> (63+) 9924401097</a></li>
                        <li><a href=""><i class="fa-brands fa-telegram"></i> (63+) 9924401097</a></li>
                        <li><a href=""><i class="fa-solid fa-envelope"></i>infinitechAdCorp@gmail.com</a></li>
                    </ul>

                    <ul class="list-social">
                        <li><a href="https://www.facebook.com/profile.php?id=61562146592005"><i class="fa-brands fa-facebook-f fa-lg"></i></a></li>
                        <li><a href=""><i class="fa-brands fa-instagram fa-lg"></i></a></li>
                        <li><a href=""><i class="fa-brands fa-x-twitter fa-lg"></i></a></li>
                    </ul>

                </div>
            </div>
       
    </div>
    @include('Layout/Script')
</nav>
